...
-- =============================================================================
-- 需求 §6：「資料庫的 DDL 和 DML 請存放在專案下的 \DB 資料夾內提供」
--
...
--
-- created_at 在此刻意指定，這與 AC-14 不衝突：
--   AC-14 規範的是「正式流程中發佈時間由 DB 層產生」，也就是 sp_post_create
--   不接受呼叫端傳入時間——該 SP 至今仍沒有時間參數，這條規則沒有被放寬。
--   種子資料是驗證用的佈置，直接寫表；若全部沿用 CURRENT_TIMESTAMP，
--   四篇會落在同一秒，排序（D-14）與相對時間顯示在畫面上就完全看不出效果。
--
-- 時間刻意讓相對時間的四種級距各出現一次：
--   12 分鐘前 → 「12 分鐘前」
--    5 小時前 → 「5 小時前」
...
-- 用於驗證：
--   1. 交易內 post 與 comment 同時被標記（需求 §6 Transaction 要求）
