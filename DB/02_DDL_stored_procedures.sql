...
-- =============================================================================
-- 需求 §6：「透過 Stored Procedure 存取資料庫」— 應用程式資料層不直接下 SQL。
-- 需求 §6：「需防止 SQL Injection」
--
...
--   需求未定義發文的排序規則（原 NG-2 / F004 OQ-2），由 owner 補為「最新在最上面」
...
-- -----------------------------------------------------------------------------
--   需求 §6：「需同時異動多個資料表時，請實作 Transaction，避免資料錯亂」
--
...
--   全案將不存在任何跨表寫入，需求 §6 的 Transaction 要求無處落地。
