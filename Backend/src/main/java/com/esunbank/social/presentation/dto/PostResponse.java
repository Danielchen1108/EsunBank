 ...
 * 發文回應（列表與編輯共用）。
 *
 * <p><b>含留言：</b>「列出所有發文」是否帶出留言需求未定義，
 * 原決定為不帶出，後由 後來追加。留言巢狀於 {@link #comments()}，
 * 讓前端一次請求就取得整個畫面所需的資料，不必逐篇再查（N+1）。
 *
 * <p>沒有留言的發文回傳空陣列而非 null——{@code []} 與 {@code null} 在前端得分開處理，
 * 但語意上並無差別，統一為空陣列可省掉那個判斷。
 *
 * <p><b>不含 is_deleted：</b>回應中的發文必然是未刪除的（的讀取過濾在 SP 內），
 * 帶出這個旗標只會誘使前端自行判斷，反而製造漏過濾的機會。
 *
 * <p><b>不含 image：</b>{@code post.image} 保留於 schema（需求規格），但無上傳功能，
 * API 亦不開放填寫（見 {@link CreatePostRequest}）。回傳一個永遠為 null 的欄位
 * 只會讓前端誤以為有圖片可顯示。
 *
 * <p><b>XSS：</b>{@code content} 為使用者輸入且會回顯，後端原樣儲存與回傳，
