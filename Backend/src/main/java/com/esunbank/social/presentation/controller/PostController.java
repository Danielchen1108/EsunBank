...
public class PostController {
    ...
    public ResponseEntity<CreatePostResponse> create(
        ...
        if (currentUser == null) {
            // 正常情況不可達：SecurityConfig 已要求本端點需驗證（負責）。
     ...
     * 列出所有發文。
     *
     * <p>已軟刪除的發文不會出現——過濾條件在 {@code sp_post_list} 內。
     *
     * <p><b>每則發文一併帶出其留言</b>（不帶出，後由 後來追加）——
     * 一次請求就取得畫面所需的全部資料，前端不必逐篇再查留言（N+1）。
     * 沒有留言的發文回傳空陣列。
     *
     * <p>留言依時間排序（順序即對話），但<b>發文本身不排序、不分頁</b>——
     * 需求未定義發文的排序規則，依 判定原則 R-3 不實作。
     */
    @GetMapping
     ...
     * 刪除發文。
     *
     * <p>軟刪除，並於同一交易內連動軟刪除其留言——
     * 需求「需同時異動多個資料表時，請實作 Transaction」的落地點。
     *
